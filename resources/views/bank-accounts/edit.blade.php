            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="bank_name" value="{{ old('bank_name', $bankAccount->bank_name) }}" required class="mt-1 block w-full erp-input">
                    ...
                    <div>
                        ...
                        <select name="account_type" required class="mt-1 block w-full erp-input">
                            @foreach(['checking', 'savings', 'fixed_deposit'] as $type)
                                <option value="{{ $type }}" @selected(old('account_type', $bankAccount->account_type) === $type)>{{ ucfirst(str_replace('_', ' ', $type)) }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" name="opening_balance" value="{{ old('opening_balance', $bankAccount->opening_balance) }}" class="mt-1 block w-full erp-input">
                        @error('opening_balance') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-slate-700">Chart of Account</label>
