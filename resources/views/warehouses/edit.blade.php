<x-app-layout>
    <x-slot name="header">{{ __('Edit Warehouse') }}: {{ $warehouse->name }}</x-slot>
    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('warehouses.update', $warehouse) }}" method="POST">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $warehouse->name) }}" required class="mt-1 block w-full erp-input">
                        @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="code" class="block text-sm font-medium text-slate-700">Code</label>
                        <input type="text" name="code" id="code" value="{{ old('code', $warehouse->code) }}" required class="mt-1 block w-full erp-input">
                    ...
                    <div>
                        ...
                        <input type="text" name="location" id="location" value="{{ old('location', $warehouse->location) }}" class="mt-1 block w-full erp-input">
                        @error('location') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="inline-flex items-center">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $warehouse->is_active) ? 'checked' : '' }} class="erp-input">
                            <span class="ml-2 text-sm text-slate-700">Active</span>
                        </label>
                    </div>
                    <div class="md:col-span-2">
                        <label for="description" class="block text-sm font-medium text-slate-700">Description</label>
                        <textarea name="description" id="description" rows="3" class="mt-1 block w-full erp-input">{{ old('description', $warehouse->description) }}</textarea>
            ...
            <div class="flex justify-end mb-8">
                ...
                <button type="submit" class="inline-flex items-center px-4 py-2 erp-btn-primary">Update Warehouse</button>
            </div>
        </form>
    </div>
</x-app-layout>
