    ...
    <div class="max-w-7xl mx-auto">
        <div class="mb-6 border-b border-slate-200">
            <nav class="flex space-x-4 -mb-px">
                @foreach (['all' => 'All', 'draft' => 'Draft', 'posted' => 'Posted', 'approved' => 'Approved', 'reversed' => 'Reversed'] as $key => $label)
        ...
        <x-table-card :empty="count($entries) === 0" emptyMessage="No journal entries found." colspan="8">
            <thead>
                <tr>
                    ...
                    <th class="px-6 py-3 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Debit Total</th>
                    <th class="px-6 py-3 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Credit Total</th>
            ...
            <tbody class="divide-y divide-slate-50">
                @forelse ($entries as $entry)
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600 max-w-xs truncate">{{ $entry->description }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800 text-right font-medium">{{ number_format($entry->total_debit, 2) }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800 text-right font-medium">{{ number_format($entry->total_credit, 2) }}</td>
