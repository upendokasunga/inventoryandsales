<x-app-layout>
    <x-slot name="header">{{ __('Edit Store Request') }}: {{ $storeRequest->request_number }}</x-slot>
    <div class="max-w-7xl mx-auto">
        <form action="{{ route('store-requests.update', $storeRequest) }}" method="POST">
            @csrf
            @method('PATCH')
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="source_warehouse_id" required class="mt-1 block w-full erp-input">
                            ...
                            @foreach ($warehouses as $w)
                                <option value="{{ $w->id }}" {{ old('source_warehouse_id', $storeRequest->source_warehouse_id) == $w->id ? 'selected' : '' }}>{{ $w->name }} ({{ $w->code }})</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="destination_warehouse_id" required class="mt-1 block w-full erp-input">
                            <option value="">Select</option>
                            @foreach ($warehouses as $w)
                                <option value="{{ $w->id }}" {{ old('destination_warehouse_id', $storeRequest->destination_warehouse_id) == $w->id ? 'selected' : '' }}>{{ $w->name }} ({{ $w->code }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="reason" rows="3" class="mt-1 block w-full erp-input">{{ old('reason', $storeRequest->reason) }}</textarea>
                        ...
                        @forelse ($storeRequest->items as $index => $item)
                            <div class="item-row grid grid-cols-12 gap-3 mb-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
                                ...
                                <div class="col-span-3">
                                    ...
                                    <input type="number" step="0.001" min="0.001" name="items[{{ $index }}][quantity_requested]" value="{{ old('items.'.$index.'.quantity_requested', $item->quantity_requested) }}" required class="block w-full erp-input text-sm">
                                </div>
            ...
            <div class="flex justify-end mb-8">
                ...
                <button type="submit" class="inline-flex items-center px-4 py-2 erp-btn-primary">Update Store Request</button>
            </div>
        </form>
    </div>
    @push('scripts')
    <script>
        let itemIndex = {{ count($storeRequest->items) }};
