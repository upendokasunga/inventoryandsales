<x-app-layout>
    <x-slot name="header">{{ __('Create Branch') }}</x-slot>
    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('branches.store') }}" method="POST">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone') }}" class="mt-1 block w-full erp-input">
                        @error('phone') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="email" class="block text-sm font-medium text-slate-700">Email</label>
                        <input type="email" name="email" id="email" value="{{ old('email') }}" class="mt-1 block w-full erp-input">
            ...
            <div class="flex justify-end mb-8">
                ...
                <button type="submit" class="inline-flex items-center px-4 py-2 erp-btn-primary">Create Branch</button>
            </div>
        </form>
    </div>
</x-app-layout>
