<x-app-layout>
    <x-slot name="header">{{ __('Sales Import Preview') }}</x-slot>

    <div class="max-w-7xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            <a href="{{ route('data-migration.sales.upload') }}" class="text-slate-400 hover:text-slate-600">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </a>
            <div class="flex-1">
                <h2 class="text-lg font-semibold text-slate-800">Sales Import Preview</h2>
        ...
        {{-- Import Settings --}}
        <form action="{{ route('data-migration.sales.import') }}" method="POST">
            @csrf
            <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="warehouse_id" class="block text-sm font-medium text-slate-700">Store/Warehouse <span class="text-red-500">*</span></label>
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-3 py-3 text-left text-xs font-medium text-slate-500 uppercase">Customer</th>
                                <th class="px-3 py-3 text-left text-xs font-medium text-slate-500 uppercase">Product</th>
                                <th class="px-3 py-3 text-left text-xs font-medium text-slate-500 uppercase">SKU</th>
                                <th class="px-3 py-3 text-right text-xs font-medium text-slate-500 uppercase">Qty</th>
                                <th class="px-3 py-3 text-right text-xs font-medium text-slate-500 uppercase">Unit Price</th>
                                <th class="px-3 py-3 text-right text-xs font-medium text-slate-500 uppercase">Total</th>
                                <th class="px-3 py-3 text-left text-xs font-medium text-slate-500 uppercase">Date</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @foreach ($sales as $row)
                                ...
                                <tr class="{{ !empty($row['errors']) ? 'bg-red-50/50' : '' }}">
                                    ...
                                    <td class="px-3 py-2.5 text-sm font-medium text-slate-800">{{ $row['customer'] }}</td>
                                    <td class="px-3 py-2.5 text-sm text-slate-600">{{ $row['product'] }}</td>
                                    <td class="px-3 py-2.5 text-sm text-slate-600">{{ $row['sku'] ?: '—' }}</td>
                                    <td class="px-3 py-2.5 text-sm text-slate-800 text-right">{{ number_format($row['quantity'] ?? 0, 0) }}</td>
                                    <td class="px-3 py-2.5 text-sm text-slate-800 text-right">{{ number_format($row['unit_price'] ?? 0, 0) }}</td>
                                    <td class="px-3 py-2.5 text-sm text-slate-800 text-right font-medium">{{ number_format($total, 0) }}</td>
                                    <td class="px-3 py-2.5 text-sm text-slate-600">{{ $row['date'] }}</td>
            ...
            <div class="flex justify-end mb-8">
                <a href="{{ route('data-migration.sales.upload') }}" class="mr-3 erp-btn-secondary">Re-upload File</a>
                <button type="submit" class="erp-btn-primary" {{ $errorCount === $totalCount ? 'disabled' : '' }}>
                    ...
                    Import {{ $totalCount - $errorCount }} Valid Sale(s)
                </button>
            </div>
        </form>
    </div>
