<x-app-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <div class="flex justify-between items-start mb-6">
                    ...
                    <span class="px-3 py-1 text-sm rounded-full font-medium
                        @if($purchaseReturn->status === 'pending_approval') bg-orange-100 text-orange-700
                ...
                <div class="border-t border-slate-200 pt-4 text-right">
                    ...
                    <p class="text-xl font-bold text-primary">TSh {{ number_format($purchaseReturn->total_amount, 2) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-4">
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-3">
                ...
                @if($purchaseReturn->status === 'pending_approval')
                    <form action="{{ route('purchase-returns.approve', $purchaseReturn) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full erp-btn-primary">Approve & Deduct Stock</button>
                    </form>
                    <form action="{{ route('purchase-returns.reject', $purchaseReturn) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full erp-btn-danger">Reject</button>
                    </form>
                @endif
                ...
                @if($purchaseReturn->status === 'approved')
                    <form action="{{ route('purchase-returns.complete', $purchaseReturn) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full erp-btn-primary">Mark Completed</button>
                    </form>
                @endif
            </div>
            ...
            @if($purchaseReturn->purchaseOrder && in_array($purchaseReturn->status, ['approved', 'completed']))
                @php $po = $purchaseReturn->purchaseOrder; @endphp
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
                    <h3 class="text-sm font-semibold text-slate-700 mb-3">PO Payment Status</h3>
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            <span class="text-slate-500">PO Total</span>
                            <span class="font-medium">TSh {{ number_format($po->total_amount, 2) }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-500">Amount Paid</span>
                            <span class="font-medium">TSh {{ number_format($po->amount_paid, 2) }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-500">Balance Due</span>
                            <span class="font-medium {{ $po->balance_due > 0 ? 'text-red-600' : 'text-green-600' }}">
                                TSh {{ number_format($po->balance_due, 2) }}
                            </span>
                        </div>
                    </div>
                </div>
