<x-app-layout>
    <x-slot name="header">Cash Transfer: {{ $transfer->reference }}</x-slot>

    <div class="max-w-2xl mx-auto">
        <div class="mb-4"><a href="{{ route('cash-transfers.index') }}" class="erp-btn-secondary">Back to List</a></div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 p-6 mb-6">
            <div class="grid grid-cols-2 gap-4 text-sm">
                ...
                @if($transfer->exchange_rate && $transfer->exchange_rate != 1)
                    <div><span class="text-slate-500">FX Rate:</span> <span class="font-mono">{{ $transfer->exchange_rate }}</span></div>
                    <div><span class="text-slate-500">Converted:</span> <span class="font-mono">TSh {{ number_format($transfer->amount * $transfer->exchange_rate, 2) }}</span></div>
                @endif
        ...
        @if($transfer->status === 'pending')
            <div class="flex gap-3">
                <form action="{{ route('cash-transfers.approve', $transfer) }}" method="POST" onsubmit="return confirm('Approve this transfer?');">
                    @csrf
                    <button type="submit" class="erp-btn-primary">Approve</button>
                </form>
                <form action="{{ route('cash-transfers.reject', $transfer) }}" method="POST" onsubmit="return confirm('Reject?');">
                    @csrf
