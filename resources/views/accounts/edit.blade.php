<x-app-layout>
    <x-slot name="header">{{ __('Edit Account') }}: {{ $account->name }}</x-slot>
    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('accounts.update', $account) }}" method="POST">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code', $account->code) }}" required class="mt-1 block w-full erp-input">
                        @error('code') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="name" class="block text-sm font-medium text-slate-700">Name</label>
                        <input type="text" name="name" id="name" value="{{ old('name', $account->name) }}" required class="mt-1 block w-full erp-input">
                    ...
                    <div>
                        ...
                        <select name="category" id="category" required class="mt-1 block w-full erp-input">
                            ...
                            <option value="current" {{ old('category', $account->category) == 'current' ? 'selected' : '' }}>Current</option>
                            <option value="non_current" {{ old('category', $account->category) == 'non_current' ? 'selected' : '' }}>Non-Current</option>
                            <option value="fixed" {{ old('category', $account->category) == 'fixed' ? 'selected' : '' }}>Fixed</option>
                            <option value="intangible" {{ old('category', $account->category) == 'intangible' ? 'selected' : '' }}>Intangible</option>
                            <option value="owner" {{ old('category', $account->category) == 'owner' ? 'selected' : '' }}>Owner</option>
                            <option value="retained" {{ old('category', $account->category) == 'retained' ? 'selected' : '' }}>Retained</option>
                            <option value="operating" {{ old('category', $account->category) == 'operating' ? 'selected' : '' }}>Operating</option>
                            <option value="non_operating" {{ old('category', $account->category) == 'non_operating' ? 'selected' : '' }}>Non-Operating</option>
                            <option value="administrative" {{ old('category', $account->category) == 'administrative' ? 'selected' : '' }}>Administrative</option>
                            <option value="selling" {{ old('category', $account->category) == 'selling' ? 'selected' : '' }}>Selling</option>
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3" class="mt-1 block w-full erp-input">{{ old('description', $account->description) }}</textarea>
            ...
            <div class="flex justify-end mb-8">
                ...
                <button type="submit" class="inline-flex items-center px-4 py-2 erp-btn-primary">Update Account</button>
            </div>
        </form>
    </div>
</x-app-layout>
