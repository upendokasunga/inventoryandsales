<x-app-layout>
    <x-slot name="header">{{ __('Create Cost Centre') }}</x-slot>
    ...
    <div class="max-w-4xl mx-auto">
        <form action="{{ route('cost-centers.store') }}" method="POST">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" required
                            class="mt-1 block w-full erp-input" placeholder="e.g. Marketing, IT Department">
                        @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="code" class="block text-sm font-medium text-slate-700">Code</label>
                        <input type="text" name="code" id="code" value="{{ old('code') }}"
                            class="mt-1 block w-full erp-input" placeholder="e.g. MKT, IT">
                        @error('code') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div class="md:col-span-2">
                        <label for="description" class="block text-sm font-medium text-slate-700">Description</label>
                        <textarea name="description" id="description" rows="3"
                            class="mt-1 block w-full erp-input" placeholder="Optional description of this cost centre">{{ old('description') }}</textarea>
            ...
            <div class="flex justify-end">
                ...
                <button type="submit" class="erp-btn-primary">Create Cost Centre</button>
            </div>
        </form>
    </div>
</x-app-layout>
