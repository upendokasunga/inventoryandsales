<x-app-layout>
    <x-slot name="header">{{ __('Edit Stock Transfer') }}: {{ $stockTransfer->transfer_number }}</x-slot>
    <div class="max-w-7xl mx-auto">
        <form action="{{ route('stock-transfers.update', $stockTransfer) }}" method="POST">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 mb-6">
                <div class="px-6 py-4 border-b border-slate-200/60"><h3 class="text-lg font-semibold text-slate-800">Transfer Details</h3></div>
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="reason" rows="3" class="mt-1 block w-full erp-input">{{ old('reason', $stockTransfer->reason) }}</textarea>
                    ...
                    <div id="items-container">
                        @forelse ($stockTransfer->items as $index => $item)
                            <div class="item-row grid grid-cols-12 gap-3 mb-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
                                <div class="col-span-5">
                                    ...
                                    <select name="items[{{ $index }}][product_id]" required class="block w-full erp-input text-sm">
                                        <option value="">Select Product</option>
                                        @foreach ($products as $p)
                                            <option value="{{ $p->id }}" {{ old('items.'.$index.'.product_id', $item->product_id) == $p->id ? 'selected' : '' }}>{{ $p->sku }} - {{ $p->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-span-2">
                                    <label class="block text-xs font-medium text-slate-600 mb-1">Quantity</label>
                                    <input type="number" step="0.001" min="0.001" name="items[{{ $index }}][quantity_transferred]" value="{{ old('items.'.$index.'.quantity_transferred', $item->quantity_transferred) }}" required class="block w-full erp-input text-sm">
                                </div>
                                <div class="col-span-3">
                                    <label class="block text-xs font-medium text-slate-600 mb-1">Unit Cost</label>
                                    <input type="number" step="0.01" min="0" name="items[{{ $index }}][unit_cost]" value="{{ old('items.'.$index.'.unit_cost', $item->unit_cost) }}" class="block w-full erp-input text-sm">
                                </div>
                        ...
                        @empty
                            <div class="item-row grid grid-cols-12 gap-3 mb-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
                                ...
                                <div class="col-span-2">
                                    <label class="block text-xs font-medium text-slate-600 mb-1">Quantity</label>
                                    <input type="number" step="0.001" min="0.001" name="items[0][quantity_transferred]" value="1" required class="block w-full erp-input text-sm">
                                </div>
                                <div class="col-span-3">
                                    <label class="block text-xs font-medium text-slate-600 mb-1">Unit Cost</label>
                                    <input type="number" step="0.01" min="0" name="items[0][unit_cost]" value="0" class="block w-full erp-input text-sm">
                                </div>
                                <div class="col-span-1 pt-5">
                                    <button type="button" class="remove-item text-red-500 hover:text-red-700" title="Remove"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                                </div>
            ...
            <div class="flex justify-end mb-8">
                <a href="{{ route('stock-transfers.index') }}" class="mr-4 inline-flex items-center px-4 py-2 erp-btn-secondary">Cancel</a>
                <button type="submit" class="inline-flex items-center px-4 py-2 erp-btn-primary">Update Stock Transfer</button>
            </div>
        </form>
    </div>
    @push('scripts')
    <script>
        let itemIndex = {{ count($stockTransfer->items) }};
